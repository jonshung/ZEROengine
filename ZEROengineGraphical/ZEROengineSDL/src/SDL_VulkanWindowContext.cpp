...
namespace ZEROengine {
    ...
    ZEROResult SDL_VulkanWindowContext::getSurface(const VkInstance& vk_instance, VkSurfaceKHR* dest) {
        ...
        if(SDL_Vulkan_CreateSurface(this->window_handler, vk_instance, nullptr, dest) < 0) {
            return { ZERO_VULKAN_CREATE_SURFACE_FAILED, (std::string(SDL_GetError())) };
        }
    ...
    bool SDL_VulkanWindowContext::resized() {
        return (this->window_event.type == SDL_EVENT_WINDOW_RESIZED);
    }
