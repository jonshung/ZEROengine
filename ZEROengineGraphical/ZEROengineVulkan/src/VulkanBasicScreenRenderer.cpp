...
void VulkanBasicScreenRenderer::initVulkanRenderer(const VulkanRendererSettings &settings, VulkanContext &vulkan_context, const uint32_t &queue_family_index) {
    if(this->vk_surface == VK_NULL_HANDLE) {
        throw std::runtime_error("VulkanBasicScreenRenderer::initVulkanRenderer() failed, error: No surface has been binded");
    }
...
void VulkanBasicScreenRenderer::initSwapChain() {
    VulkanContext::SwapChainSupportDetails swap_chain_support = this->vulkan_context->querySwapChainSupport(this->vk_surface, this->vulkan_context->getPhysicalDevice());
...
VkExtent2D VulkanBasicScreenRenderer::selectSwapChainExtent(const VkSurfaceCapabilitiesKHR &capabilities) {
    ...
    VkExtent2D actual_extent = {
            static_cast<uint32_t>(this->frame_width),
            static_cast<uint32_t>(this->frame_height)
    };
...
void VulkanBasicScreenRenderer::draw(VulkanGraphicsPipelineBuffer *g_pipeline_buffer) {
    ...
    vkCmdBindVertexBuffers(
        this->frame_cmd_buffers[current_frame_index].buffer, // command buffer
        0,  // first binding
...
void VulkanBasicScreenRenderer::handleResize(const uint32_t &new_frame_width, const uint32_t &new_frame_height) {
    this->setFrameDimensions(new_frame_width, new_frame_height);
