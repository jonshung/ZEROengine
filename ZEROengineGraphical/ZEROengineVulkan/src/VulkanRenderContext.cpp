...
namespace ZEROengine {
    ...
    void VulkanRenderContext::recordRenderPassCommandBuffer(
        ...
        render_pass_begin.framebuffer = recording.vk_framebuffer;

        render_pass_begin.renderPass = recording.vk_renderpass;
        render_pass_begin.renderArea.extent = recording.vk_render_area;
        ...
        vkCmdExecuteCommands(primary_cmd_buffer, 1, &recording.cmd);
        vkCmdEndRenderPass(primary_cmd_buffer);
    }

    void VulkanRenderContext::recordTransferCommandBuffer(const uint32_t &target_index, const VulkanTransferRecordingInfo &recording) {
        VkCommandBuffer& primary_cmd_buffer = this->vk_primary_cmd_buffer[target_index];
        vkCmdExecuteCommands(primary_cmd_buffer, 1, &recording.cmd);
    }
    ...
    void VulkanRenderContext::present(const VkPresentInfoKHR &present_info) {
